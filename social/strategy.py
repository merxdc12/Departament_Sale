...
def _validate(signals: SocialSignals) -> None:
    for name, value in (
        ...
        ("content_fit", signals.content_fit),
        ("purchase_intent", signals.purchase_intent),
        ("engagement_fit", signals.engagement_fit),
    ):
        if not 0 <= value <= 100:
...
def build_social_plan(policy: SocialPolicy, signals: SocialSignals, *, commercial: bool = False) -> SocialPlan:
    _validate(signals)
    score = round(
        signals.audience_fit * 0.35
        + signals.content_fit * 0.30
        + signals.purchase_intent * 0.20
        + signals.engagement_fit * 0.15
    )
    ...
    if commercial:
        ...
        fmt = {
            "PINTEREST": "PRODUCT_PIN",
            "INSTAGRAM": "REEL_OR_CAROUSEL",
            "FACEBOOK": "POST_OR_REEL",
            "THREADS": "TEXT_THREAD",
        }[policy.platform]
        return SocialPlan(
            policy.platform,
            ...
            score,
            fmt,
            policy.affiliate_links_allowed,
            policy.disclosure_required,
            policy.human_approval_required,
            "Run a transparent commercial content test.",
            ("Commercial relationship must be disclosed where required.", "No artificial engagement or account manipulation."),
        )
    ...
    return SocialPlan(
        ...
        False,
        policy.human_approval_required,
        "Build organic reach with useful platform-native content.",
        ("Start organic-first and measure performance before paid escalation.",),
    )
